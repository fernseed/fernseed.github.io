...
This is a [static](http://en.wikipedia.org/wiki/Static_web_page) website---the content you're reading now is generated once, for all users, then served in exactly the same way each time to each visitor. That's slightly different from the current internet norm of generating content as it's requested, possibly on a targeted per-user basis, and probably using a database or some server-side business logic. Think of Wordpress and its plugins for examples of the latter, while [Pelican](http://blog.getpelican.com), [Jekyll][jekyll], and [Octopress](http://octopress.org) would be examples of the former[^fn1].

Even though the back-end mechanics are very different, there's not as much variation in terms of how the site is actually put together when writing posts under either approach, since today's static and dynamic websites typically use some kind of [CMS](http://en.wikipedia.org/wiki/Content_management_system) or templating system to create and organise content. Just because a website is static it doesn't mean that you're constrained to writing everything in pure, flat HTML[^fn2].

1. Table of Contents
{:toc}

The basic goal of a static website is to be *complete* and *universal* at the moment it is turned into HTML for publication---a single set of generated content is all that any client browser ever sees[^fn3].
...
We'll keep our CSV structure as trivial as possible:
...
Since I use [Scrivener](http://www.literatureandlatte.com/scrivener.php) for any long-form writing I could just go to it directly for the statistics I'm looking for---it tracks those details in its Project Target metadata yet, unlike Google Docs, it does not expose that data through an API. That doesn't stop us extracting it ourselves using a script since the data we care about is held within a readable section of the document package's interior `.scrivx` file and relates to the current session[^fn9]. A 'session' refers to an automatically or manually-resetting period of time which we have defined ourselves within the project preferences, so its data will represent any words which have been written between the previous session reset up until the point that you've called your script. 
...
Scrivener, like most OS X applications[^fn13], will automatically save any changes made to a project every few seconds---the Snapshots feature even allows you to roll back to earlier auto-saved drafts (even though the app [doesn't support](http://www.literatureandlatte.com/forum/viewtopic.php?f=4&t=78&p=123475&hilit=lion+autosave#p123475) the built-in [OS X system-level versioning/reverting](http://support.apple.com/kb/PH14378) mechanism for files, since your `.scriv` document isn't really a single file at all). This means that my multi-decade muscle memory to spam Control or Command-S to save every once in a while to make sure I don't lose anything is entirely redundant. So let's repurpose that muscle memory and re-bind that shortcut to 'Sync with External Folder Now'[^fn14]:
...
At this point we can pick up any plain text files stored in the 'Drafts' folder(s) in the sync target location and run them through `wc`[^fn15]:
...
You can read more detail about the setup process for Jekyll data files on the main [Jekyll documentation site](http://jekyllrb.com/docs/datafiles/). Liquid markup isn't always the cleanest in the world, although the lack of any particularly complex syntax does mean that any logic you write is normally fairly parseable and reads much like pseudocode:
...
If you follow all of the above, the final interactive and dynamically updating chart will end up looking like this, tracking selectable periods of writing data which are updated on a daily basis:
...
It's a fairly limited example, but hopefully it illustrates the point---any visitors coming to this page will get exactly the same data presented in exactly the same way; no new data is being retrieved by the site on each of those visits or when any of the period buttons are clicked; I don't need to manually update the page content to reflect any new data which is added by scripts over time; and the graph and its buttons will behave as though it's being generated in the more traditional fashion from a database. 
...
[^fn7]: There's no reason you couldn't plug some offline Google Doc-derived stats into a static website, but we'll do something different here.
...
[^fn14]: If you're not on OS X, or just prefer having that keymap unchanged, you can choose a different keybinding of your choice. This is all about reducing friction for me, so I won't be trying to memorise a new keybinding when an old one is both redundant and semantically correct for what I now want to achieve.

[^fn15]: I'm also running them through `bc` in the below to ensure that we have only numbers in the output.

[^fn16]: Leaving aside your thoughts on how robust all that Jekyll tags logic and shell scripting might be...
